...
> 最后验证时间：2026-09-08，commit `623c25a`。
...
GitHub (main push)
   │  触发 Actions
   ▼
GitHub Actions runner（境外 Ubuntu）
   │  npm ci + vite build → dist/
   │  tar 打包 dist + server → deploy.tar.gz
   │  scp 推到服务器 /tmp
   │  ssh 解压 + npm ci + pm2 restart
   ▼
阿里云 ECS 8.137.91.87（Ubuntu 22.04）
   ├── Nginx :80  ── /  → 静态托管 /var/www/first-ancient/dist
   │              └─ /api/ → 反代 127.0.0.1:3000
   ├── Express :3000（pm2 守护，名 course-api）
   └── MySQL  course_platform 库
```
