...
上传的文件会以 base64 整串写进 LONGTEXT 列，MySQL 的 `max_allowed_packet`
必须大于请求体上限（20MB），否则大文件上传时 MySQL 会**直接掐断连接**，
后端 pm2 日志里只看到「服务器已重启」而不是一条报错：

```bash
# 写入 /etc/mysql/mysql.conf.d/mysqld.cnf 的 [mysqld] 段落后重启
#   max_allowed_packet = 20M
printf '[mysqld]\nmax_allowed_packet=20M\n' > /etc/mysql/mysql.conf.d/zz-upload.cnf
systemctl restart mysql
mysql -u root -p -e "SHOW VARIABLES LIKE 'max_allowed_packet';"   # 应为 20971520
```

后端启动时会自检这一项，不满足会在 pm2 日志里打 warning（不阻断启动）。
</details>
...
CORS_ORIGINS=http://47.114.42.189
EOF
```
...
server {
    ...
    location /api/ {
        ...
        # 上传课程标准/项目资料时前端把文件转成 base64 放进 JSON body，
        # 5MB 文件约合 6.7MB 请求体。nginx 默认只放行 1MB，不放开的话
        # 大文件会在网关这里被 413 掉，根本到不了后端，
        # 表现为「上传失败」但后端日志里什么都看不到。
        # 20m 与 server/lib/uploadLimits.js 的 MAX_UPLOAD_BODY_SIZE 对齐。
...
flowchart LR
  ...
  P4 --> P5["Actions 构建"] --> P6["scp+ssh 服务器"] --> P7["刷新 47.114.42.189"]
```
...
```
push 完 → Actions 自动构建 → scp 推服务器 → ssh 解压 + pm2 重启 → 刷新 `http://47.114.42.189` 即最新。**只推 `main` 才触发部署**，其他分支不会。
...
curl http://47.114.42.189/api/health # 健康检查
...
- **不部署 Java 后端**：项目已无 Java 后端（`java-backend/` 已删除），全部功能由 `server/` 的 Express 提供。
